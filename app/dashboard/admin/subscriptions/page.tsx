...
export default function AdminSubscriptionsPage() {
  ...
  // Optimized Filter + Sort by Email to group multiple magazines per user
  const filteredSubs = useMemo(() => {
    return subs
      ...
      .sort((a, b) => a.user.email.localeCompare(b.user.email)); // Crucial for visibility
  }, [subs, searchQuery]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-white/40 text-sm mt-2">Grouped by user email • Manage access & dates</p>
          </div>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredSubs.length > 0 ? (
                filteredSubs.map((sub, index) => {
                  const isSameUser = index > 0 && filteredSubs[index - 1].user.email === sub.user.email;
                  
                  return (
                    <tr key={sub.id} className="hover:bg-white/[0.01] transition-colors group">
                      <td className="p-5">
                        <div className={`flex flex-col transition-opacity ${isSameUser ? 'opacity-20' : 'opacity-100'}`}>
                          <span className="font-medium">{sub.user.email}</span>
                          {!isSameUser && <span className="text-[9px] text-amber-500/50 font-bold uppercase tracking-tighter">Primary Record</span>}
                        </div>
                      </td>
                      <td className="p-5">
