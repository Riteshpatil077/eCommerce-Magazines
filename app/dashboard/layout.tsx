...
export default async function DashboardLayout({
  ...
  // 1. Fetch FRESH user data from the Database to get the 'name'
      ...
      <aside className="hidden md:flex flex-col w-64 shrink-0 border-r border-white/5 bg-zinc-900/50">
        <div className="px-6 py-7 border-b border-white/5">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-serif text-lg font-bold tracking-tight text-stone-100">Pressly</span>
          </Link>
        </div>
            ...
            <div className="group relative">
              <button suppressHydrationWarning className="flex items-center gap-3 p-1.5 rounded-full hover:bg-white/5 transition-all border border-transparent hover:border-white/10">
                <div className="flex flex-col items-end hidden sm:block">
                  {/* Updated to show Real Name */}
                  <p className="text-xs font-medium text-stone-200 leading-none">{displayName}</p>
              ...
              <div className="absolute right-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                <div className="w-56 bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-1.5">
                  ...
                  <Link href="/dashboard/user/profile" className="flex items-center gap-3 px-3 py-2 text-sm text-white/70 hover:bg-white/5 hover:text-white rounded-lg transition-colors">
