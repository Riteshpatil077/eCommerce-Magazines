...
import SubscribeForm from "@/app/components/store/SubscribeForm"
...
export default async function MagazinePage({ params }: PageProps) {
  ...
  const [magazine, subscription] = await Promise.all([
    prisma.magazine.findUnique({ where: { slug } }),
    // We can't fetch subscription until we have user.id, but we can do it 
    // alongside the magazine fetch since user is already resolved.
    prisma.subscription.findFirst({
      where: {
        userId: user.id,
        magazine: { slug }, // You can join here to find by slug directly!
        ...
        <div className="flex flex-col sm:flex-row gap-8 mb-10">
          ...
          <div className="shrink-0 w-36 md:w-44">
            <div className="aspect-[3/4] rounded-xl overflow-hidden border border-white/5 shadow-2xl">
              <div className="shrink-0 w-36 md:w-44">
                {/* The parent MUST be relative for 'fill' to work */}
                <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-white/5 shadow-2xl">
                  <Image
                    src={magazine.coverImage}
                    alt={magazine.title}
                    fill
                    sizes="(max-width: 768px) 144px, 176px" // 36rem and 44rem equivalents
                    priority // This is the main image on the page, load it first!
        ...
        {subscription ? (
          ...
          <div className="relative bg-zinc-900 border border-white/5 rounded-2xl overflow-hidden">
            ...
            <div className="relative z-20 flex flex-col items-center text-center px-8 py-20">
              ...
              <SubscribeForm magazineId={magazine.id} price={magazine.price} />
