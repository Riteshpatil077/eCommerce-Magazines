...
export default async function StorePage() {
  ...
  const subscribedIds = new Set(
    subscriptions.map((sub) => sub.magazineId)
  )
...
function Section({ title, magazines, subscribedIds }: any) {
  return (
      ...
      <div className="flex space-x-5 overflow-x-auto pb-4 scrollbar-hide">
        {magazines.map((mag: any) => {
          ...
          return (
            ...
            <div
              key={mag.id}
              className="relative min-w-[200px] h-[300px] rounded-lg overflow-hidden group bg-zinc-900"
            >
              {/* Image container */}
              ...
              {/* Gradient overlay - darkens on hover to make text/buttons readable */}
              ...
              {isSubscribed && (
                <span className="absolute top-3 right-3 z-10 bg-amber-400 text-black text-[10px] font-bold px-2 py-1 rounded shadow-lg">
                  Subscribed
                </span>
              )}
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 flex flex-col justify-end">
                <h3 className="text-sm font-semibold truncate mb-3 text-white drop-shadow-md">
                  {mag.title}
                </h3>

                {/* Buttons: Only visible/interactive on hover or focus */}
                <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">

                  {/* View Button */}
                  <Link
                    href={`/store/${mag.slug}`}
                    className="block text-[10px] uppercase font-bold tracking-wider bg-white text-black py-2 text-center rounded hover:bg-gray-200 transition-colors"
                  >
                    magazine details
                  </Link>

                  {/* Add To Cart Button - Only show if NOT subscribed */}
                  {!isSubscribed ? (
                    <form action={addToCart}>
                      ...
                      <button
                        type="submit"
                        className="w-full text-[10px] uppercase font-bold tracking-wider bg-yellow-500 hover:bg-yellow-400 text-black py-2 rounded transition-colors"
                      >
                        Add To Cart
                      </button>
                    ...
                    <Link
                      href={`/store/${mag.slug}`}
                      className="block text-[10px] uppercase font-bold tracking-wider bg-amber-400 text-black py-2 text-center rounded"
                    >
                      Read Now
                    </Link>
                  )}
